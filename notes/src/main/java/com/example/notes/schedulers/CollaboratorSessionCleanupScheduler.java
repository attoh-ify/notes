...
public class CollaboratorSessionCleanupScheduler {
    ...
    public void cleanupStaleSessions() {
        Set<UUID> activeNoteIds = redisService.getActiveCollaborationNoteIds();

        if (activeNoteIds.isEmpty()) {
            return;
        }

        for (UUID noteId : activeNoteIds) {
            try {
    ...
    private void cleanupOneNote(UUID noteId) {
    ...
    private void persistAndDeleteInactiveNote(UUID noteId, String ownerEmail) {
